    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            @if($asset->condition !== 'Scrap')
                <button type="button" onclick="document.getElementById('condition').value = 'Scrap'; document.getElementById('condition').classList.add('bg-red-100', 'text-red-900'); alert('Condition set to Scrap. Please click Update Asset to save.');" class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded shadow text-sm">
                    Mark as Damaged
                </button>
            @endif
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" list="suppliers_list" name="supplier_name" id="supplier_name" value="{{ old('supplier_name', $asset->supplier->name ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Search or type new supplier...">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="delivery_date" id="delivery_date" value="{{ old('delivery_date', $asset->delivery_date ? $asset->delivery_date->format('Y-m-d') : '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                            <div>
                                <label for="gr_date" class="block text-sm font-medium text-gray-700">GR Date</label>
                                <input type="date" name="gr_date" id="gr_date" value="{{ old('gr_date', $asset->gr_date ? $asset->gr_date->format('Y-m-d') : '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="number" step="0.01" name="unit_price" id="unit_price" value="{{ old('unit_price', $asset->unit_price) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                            <div>
                                ...
                                <select name="currency" id="currency" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    <option value="AFN" {{ old('currency', $asset->currency) == 'AFN' ? 'selected' : '' }}>AFN</option>
                                    <option value="USD" {{ old('currency', $asset->currency) == 'USD' ? 'selected' : '' }}>USD</option>
                                    <option value="EURO" {{ old('currency', $asset->currency) == 'EURO' ? 'selected' : '' }}>EURO</option>
                                </select>
                            </div>
                            <div>
                                <label for="total_amount" class="block text-sm font-medium text-gray-700">Total Amount</label>
                                <input type="number" step="0.01" name="total_amount" id="total_amount" value="{{ old('total_amount', $asset->total_amount) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                        </div>
